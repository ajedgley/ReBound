...
class Window:
    ...
    def on_annotation_start(self):
    	...
    	self.controls.close()
    	annotation_object = self.setup_control_window(self.widget3d, self.pointcloud_window, self.frame_extrinsic, self.boxes, self.path_string)
    ...
    def setup_control_window(self, scene_widget, pointcloud, frame_extrinsic, boxes, path):
        ...
        self.create_box_scene(self.widget3d, self.boxes_to_render, self.frame_extrinsic)
        ...
        box_partial = functools.partial(self.add_bounding_box, widget=scene_widget, pw=pointcloud,
                                        fe=frame_extrinsic)  # necessary to add args to functions
        ...
        click_partial = functools.partial(self.get_point_depth, widget=scene_widget, boxes=boxes_available)
    ...
    def place_bounding_box(self, event, widget, pw, fe):
        if event.type == gui.MouseEvent.Type.BUTTON_DOWN:
            ...
            qtr = Quaternion([random.uniform(-0.1, 1), random.uniform(-0.1, 1), random.uniform(-0.1, 1),
                              random.uniform(0, 1)])  # Randomized rotation of box
            origin = self.widget3d.center_of_rotation# Random origin of box within existing pointcloud bounds
            size = [random.randint(1, 5), random.randint(1, 5), random.randint(1, 5)]  # Random dimensions of box
            mat = rendering.MaterialRecord()

            bounding_box = o3d.geometry.OrientedBoundingBox(origin, qtr.rotation_matrix,
                                                            size)  # Creates bounding box object
            #bounding_box.rotate(Quaternion(fe['rotation']).rotation_matrix,
            #                    [0, 0, 0])  # Frame extrinsic data necessary to display boxes
            #bounding_box.translate(np.array(fe['translation']))
            bounding_box.color = matplotlib.colors.to_rgb(
                (0, 0, 1))  # Custom bounding boxes are blue to differentiate from the existing ones

            #global box_count  # Ensures that the scene treats each box differently, letting you add multiple
            widget.scene.add_geometry(str(self.box_count), bounding_box, mat)  # Generates a box
    ...
    def get_point_depth(self, event, widget, boxes):
    ...
    def select_box(self, box_index):
        if self.previous_index != -1: #if not first box clicked "deselect" previous box
            ...
            rendering.Open3DScene.modify_geometry_material(self.widget3d.scene, self.box_indices[self.previous_index],
                                                           prev_mat)

        rendering.Open3DScene.remove_geometry(self.widget3d.scene, self.coord_frame)
        self.previous_index = box_index
        box = self.box_indices[box_index]
        origin = o3d.geometry.TriangleMesh.get_center(self.volumes_rendered[box_index])
        ...
        mat = rendering.MaterialRecord()
        mat.shader = "unlitLine"
        rendering.Open3DScene.modify_geometry_material(self.widget3d.scene, box, mat)
        self.widget3d.scene.add_geometry("coord_frame", frame, frame_mat, True)
    ...
    def create_box_scene(self, scene, boxes, extrinsics):
        ...
        mat.base_color = (0.0, 0.0, 0.0, 0.0) #sets boxes to be invisble
        ...
        for box in boxes:
            ...
            boxes_to_return.append(cube_to_add)
            scene.scene.add_geometry(cube_id, cube_to_add, mat)

        self.volumes_rendered = boxes_to_return
